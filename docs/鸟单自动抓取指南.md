...
python3 tools/fetch_from_birdreport.py \
  ...
  --location beijing_aosen_north \
  --days 7 \
  --output data/beijing_aosen_north_$(date +%Y%m%d).txt
```
- `point_aliases` 数组可列出所有需要并集的搜索词，脚本逐一抓取并自动去重。
- `default_days`：未手动指定日期时的回溯天数。
- `query_level`（可选）：查询级别，支持以下值：
  - `"point"`（默认）：地点级别，使用 `point_aliases` 查询具体地点
  - `"district"`：区县级别，查询整个区县的所有地点
  - `"city"`：城市级别，查询整个城市的所有地点
  - `"province"`：省级别，查询整个省份的所有地点
  - 当使用 `city`/`district`/`province` 级别时，`point_aliases` 可以留空
- `districts`（可选）：当 `query_level` 为 `"district"` 时，可以使用 `districts` 数组指定多个区县，系统会自动遍历查询并合并结果
...
python3 tools/run_weekly_refresh.py \
  --days 7 \
  --min-species 10 \
  --locations beijing_aosen_north beijing_yuanmingyuan shanghai_shiji_gongyuan
```
- 默认遍历 `config/birdreport_locations.json`，对每个地点调用 `fetch_from_birdreport.py`、`convert_to_csv.py` 以及 `process_new_birds.py` 中的核心函数。
- 最新一周的 Cloudinary JSON 会复制到 `feather-flash-quiz/location_birds/<城市>/<地点>/<YYMMDD>`。若去重鸟种数低于 `--min-species`（默认 10），会跳过该地点以保留上次报告。
