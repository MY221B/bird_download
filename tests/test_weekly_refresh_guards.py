...
def write_single_slug_csv(csv_file):
    csv_file.write_text(
        "# slug,chinese_name,english_name,scientific_name,wikipedia_page\n"
        "only_bird,测试鸟,Only Bird,Only bird,Only_Bird\n",
        encoding="utf-8",
    )
    return {"only_bird": {"chinese_name": "测试鸟", "english_name": "Only Bird"}}
    ...
    def assert_low_species_location_is_skipped(self, module_name):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                mock.patch.object(module, "write_csv_from_records", side_effect=write_single_slug_csv) as write_csv,
