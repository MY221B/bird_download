...
class WeeklyRefreshScriptTests(unittest.TestCase):
    ...
    def test_v2_restores_quiz_remote_after_token_injection(self):
        result = self.run_script(
            "v2_weekly_refresh_and_push.sh",
            checkout_quiz=True,
            extra_env={"FEATHER_FLASH_QUIZ_TOKEN": "test-token"},
        )

        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        git_log = (result.workdir / "git.log").read_text(encoding="utf-8")
        remote_set_url_lines = [
            line
            for line in git_log.splitlines()
            if "cwd=" in line and "args=remote set-url origin " in line
        ]
        self.assertGreaterEqual(len(remote_set_url_lines), 2, git_log)
        self.assertIn("x-access-token:test-token", remote_set_url_lines[0])
        self.assertEqual(
            remote_set_url_lines[-1].split("args=remote set-url origin ", 1)[1],
            "https://github.com/MY221B/feather-flash-quiz.git",
            git_log,
        )

    def run_script(self, script_name, checkout_quiz, extra_env=None):
        ...
        try:
            ...
            result = subprocess.run(
                ["bash", str(tools_dir / script_name)],
                cwd=workdir,
                env=env,
                text=True,
                capture_output=True,
        ...
        self.write_executable(
            ...
            textwrap.dedent(
                ...
                case "${1:-}" in
                  ...
                  remote)
                    if [[ "${2:-}" == "get-url" ]]; then
                      echo "https://github.com/MY221B/feather-flash-quiz.git"
                    fi
                    exit 0
                    ;;
                  *)
