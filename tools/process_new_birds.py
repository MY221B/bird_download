...
def update_bird_info(csv_file):
    ...
    # 从新增鸟单.txt提取中文名；自动化流程没有该文件时使用 CSV 中的名称。
    input_file = PROJECT_ROOT / "新增鸟单.txt"
    chinese_map = {
        slug: info.get('chinese_name', '')
        for slug, info in bird_map.items()
        if info.get('chinese_name')
    }

    if not input_file.exists():
        print(f"ℹ️  新增鸟单.txt 不存在，使用 CSV 中的 bird_info 更新")
    else:
        with open(input_file, 'r', encoding='utf-8') as f:
            lines = [l.strip() for l in f.readlines()]
    ...
    for slug, info in bird_map.items():
        ...
        if json_file.exists():
            ...
            if 'bird_info' not in data:
                data['bird_info'] = {}
            existing_info = data['bird_info'] if isinstance(data['bird_info'], dict) else {}
            data['bird_info']['slug'] = slug
            data['bird_info']['chinese_name'] = (
                chinese_map.get(slug) or info.get('chinese_name') or existing_info.get('chinese_name', '')
            )
            data['bird_info']['english_name'] = info.get('english_name') or existing_info.get('english_name', '')
            data['bird_info']['scientific_name'] = (
                info.get('scientific_name') or existing_info.get('scientific_name', '')
            )
