...
ORIGINAL_QUIZ_REMOTE_URL=""

quiz_worktree_available() {
  [[ -d "${QUIZ_DIR}" && ( -d "${QUIZ_DIR}/.git" || -f "${QUIZ_DIR}/.git" ) ]]
}

restore_quiz_remote() {
  if [[ -n "${ORIGINAL_QUIZ_REMOTE_URL}" ]]; then
    git -C "${QUIZ_DIR}" remote set-url origin "${ORIGINAL_QUIZ_REMOTE_URL}" 2>/dev/null || true
  fi
}
...
if quiz_worktree_available; then
  ...
  echo "⚠️  feather-flash-quiz 未检出为 Git worktree，跳过 Lovable 同步"
fi
echo ""
...
if quiz_worktree_available; then
  ...
  # 在 Cloud Agent 环境中临时注入 token 到子模块 remote（本地无此环境变量时跳过）
  if [[ -n "${FEATHER_FLASH_QUIZ_TOKEN:-}" ]]; then
    ORIGINAL_QUIZ_REMOTE_URL="$(git remote get-url origin 2>/dev/null || true)"
    trap restore_quiz_remote EXIT
    git remote set-url origin "https://x-access-token:${FEATHER_FLASH_QUIZ_TOKEN}@github.com/MY221B/feather-flash-quiz.git"
    echo "✅ feather-flash-quiz remote URL 已临时注入 token"
  fi
  ...
  echo "⚠️  feather-flash-quiz 未检出为 Git worktree，跳过子模块提交"
fi
