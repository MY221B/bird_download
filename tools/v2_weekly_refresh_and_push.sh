...
bash "${REPO_ROOT}/tools/sync_from_lovable.sh"
echo "✅ Lovable 同步完成"
...
# 在 Cloud Agent 环境中使用 token 推送子模块，但不写入 remote 配置。
if [[ -n "${FEATHER_FLASH_QUIZ_TOKEN:-}" ]]; then
  ...
  echo "✅ feather-flash-quiz push token 已就绪"
fi
  ...
  if git diff --cached --quiet; then
    ...
    if ! git pull --rebase origin "${current_branch}"; then
      if git status | grep -q "rebase in progress"; then
        ...
        echo "❌ feather-flash-quiz pull/rebase 失败"
      fi
      exit 1
    fi
