...
push_quiz() {
  ...
  if [[ -n "${FEATHER_FLASH_QUIZ_TOKEN:-}" ]]; then
    git push --quiet "https://x-access-token:${FEATHER_FLASH_QUIZ_TOKEN}@github.com/MY221B/feather-flash-quiz.git" "${refspec}"
  else
...
if [[ -n "${FEATHER_FLASH_QUIZ_TOKEN:-}" ]]; then
  ...
  echo "✅ feather-flash-quiz push 将使用临时 token URL"
fi

node scripts/generate-location-birds-manifest.js > /dev/null 2>&1
if [[ -z "$(git status --porcelain)" ]]; then
  echo "✅ feather-flash-quiz 无新改动，跳过子模块提交"
else
  git add -A
  if git diff --cached --quiet; then
    ...
    push_quiz main
    push_quiz main:develop_lovable
