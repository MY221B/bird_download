...
# shellcheck source=tools/quiz_git_helpers.sh
source "${REPO_ROOT}/tools/quiz_git_helpers.sh"
...
ensure_quiz_submodule "${REPO_ROOT}" "${QUIZ_DIR}"
cd "${QUIZ_DIR}"
...
if [[ "${QUIZ_HAS_CHANGES}" == "1" ]]; then
  ...
  if ! quiz_git_with_auth pull --rebase origin "${current_branch}" 2>/dev/null; then
    if git status | grep -q "rebase in progress"; then
      echo "❌ feather-flash-quiz rebase 冲突，请手动解决"
      exit 1
    fi
  fi
  quiz_git_with_auth push --quiet origin main
  quiz_git_with_auth push --quiet origin main:develop_lovable
