...
FEATHER_FLASH_QUIZ_PUSH_REMOTE="origin"
if [[ -n "${FEATHER_FLASH_QUIZ_TOKEN:-}" ]]; then
  FEATHER_FLASH_QUIZ_PUSH_REMOTE="https://x-access-token:${FEATHER_FLASH_QUIZ_TOKEN}@github.com/MY221B/feather-flash-quiz.git"
...
if [[ -z "$(git status --porcelain)" ]]; then
  ...
  if git diff --cached --quiet; then
    ...
    git push --quiet "${FEATHER_FLASH_QUIZ_PUSH_REMOTE}" main
    git push --quiet "${FEATHER_FLASH_QUIZ_PUSH_REMOTE}" main:develop_lovable
