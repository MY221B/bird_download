...
QUIZ_ORIGIN_URL=""
QUIZ_TOKEN_INJECTED=0

restore_quiz_remote() {
  if [[ "${QUIZ_TOKEN_INJECTED}" -eq 1 && -n "${QUIZ_ORIGIN_URL}" ]]; then
    git -C "${QUIZ_DIR}" remote set-url origin "${QUIZ_ORIGIN_URL}" 2>/dev/null || true
  fi
}

if [[ -d "${QUIZ_DIR}" && -e "${QUIZ_DIR}/.git" ]]; then
  cd "${QUIZ_DIR}"

  # 在 Cloud Agent 环境中注入 token 到子模块 remote（本地无此环境变量时跳过）
  if [[ -n "${FEATHER_FLASH_QUIZ_TOKEN:-}" ]]; then
    QUIZ_ORIGIN_URL="$(git remote get-url origin 2>/dev/null || echo "")"
    git remote set-url origin "https://x-access-token:${FEATHER_FLASH_QUIZ_TOKEN}@github.com/MY221B/feather-flash-quiz.git"
    QUIZ_TOKEN_INJECTED=1
  ...
  echo "⚠️  feather-flash-quiz 不是独立 Git 工作区，跳过 quiz 提交"
fi
