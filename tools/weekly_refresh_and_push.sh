...
if [[ -z "$(git status --porcelain)" ]]; then
  echo "✅ feather-flash-quiz has no changes; skipping submodule commit."
else
  echo "🗂️  Staging changes under feather-flash-quiz..."
  git add -A
  if git diff --cached --quiet; then
    echo "⚠️  Nothing new to commit after staging; skipping submodule commit."
  else
    # 检查 git 用户配置
...
if [[ "${PWD}" != "${REPO_ROOT}" ]]; then
  echo "❌ Failed to return to repository root: ${REPO_ROOT}"
  exit 1
fi

if [[ "${PWD}" == "${QUIZ_DIR}" ]]; then
  echo "❌ Refusing to handle main repository changes while still inside feather-flash-quiz"
  exit 1
fi
