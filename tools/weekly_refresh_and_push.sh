...
if [[ -d "${QUIZ_DIR}" && ( -d "${QUIZ_DIR}/.git" || -f "${QUIZ_DIR}/.git" ) ]]; then
  cd "${QUIZ_DIR}"

  GIT_AUTH_CONFIG=()
  if [[ -n "${FEATHER_FLASH_QUIZ_TOKEN:-}" ]]; then
    AUTH_HEADER="$(printf 'x-access-token:%s' "${FEATHER_FLASH_QUIZ_TOKEN}" | base64 | tr -d '\n')"
    GIT_AUTH_CONFIG=(-c "http.https://github.com/MY221B/feather-flash-quiz.git.extraheader=Authorization: Basic ${AUTH_HEADER}")
    echo "✅ feather-flash-quiz will use temporary token authentication."
  fi
  ...
  if [[ -z "$(git status --porcelain)" ]]; then
    ...
    if git diff --cached --quiet; then
      ...
      if git "${GIT_AUTH_CONFIG[@]}" pull --rebase origin "${current_branch}" 2>&1; then
      ...
      git "${GIT_AUTH_CONFIG[@]}" push --quiet origin main
      echo "🚀 Pushing feather-flash-quiz to origin/develop_lovable..."
      git "${GIT_AUTH_CONFIG[@]}" push --quiet origin main:develop_lovable
      echo "✨ feather-flash-quiz pushed successfully (main + develop_lovable)."
    fi
  fi
else
  echo "✅ feather-flash-quiz is not an independent Git worktree; skipping quiz commit."
fi
