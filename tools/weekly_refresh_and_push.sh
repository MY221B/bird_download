...
source "${REPO_ROOT}/tools/quiz_git_utils.sh"
...
if quiz_git_is_independent_repo "${QUIZ_DIR}"; then
  echo "🔄 更新 location_birds 路径清单 (manifest)..."
  cd "${QUIZ_DIR}"
  node scripts/generate-location-birds-manifest.js
  if [[ -z "$(git status --porcelain)" ]]; then
    echo "✅ feather-flash-quiz has no changes; skipping quiz commit."
  else
    ...
    if git diff --cached --quiet; then
      ...
      if quiz_git_pull_rebase "${current_branch}" 2>&1; then
